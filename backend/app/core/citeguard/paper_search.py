...
class PaperSearcher:
    ...
    def search_arxiv(self, title: str = None, arxiv_id: str = None) -> Optional[PaperInfo]:
        """
        Search arXiv API for preprints.
        Can search by title or arxiv_id (e.g., '1602.01783' or 'arXiv:1602.01783')
        """
        try:
            ...
            for entry in entries:
                ...
                entry_title = entry_title.text.strip().replace('\n', ' ') if entry_title is not None else ''
                
                # Calculate similarity if searching by title
                if title:
                    score = self._calculate_similarity(title, entry_title)
                    if score <= best_score:
                        continue
                    best_score = score
                else:
                    score = 1.0  # Direct ID lookup
                    best_score = 1.0
                ...
                arxiv_id_extracted = arxiv_url.split('/')[-1] if arxiv_url else ''
                
                best_match = PaperInfo(
                    title=entry_title,
                    authors=authors,
                    year=year,
                    abstract=abstract,
                    doi=f"arXiv:{arxiv_id_extracted}",
                    url=arxiv_url,
                    source='arxiv',
    ...
    def verify_url(self, url: str) -> Tuple[bool, str]:
        ...
        """
        try:
            # Clean URL from LaTeX formatting
            if '\\url{' in url:
                url = url.replace('\\url{', '').rstrip('}')
